...
wxBEGIN_EVENT_TABLE(CustomButton, wxButton)
    EVT_PAINT(CustomButton::OnPaint)
        EVT_ENTER_WINDOW(CustomButton::OnMouseEnter)
            EVT_LEAVE_WINDOW(CustomButton::OnMouseLeave)
                EVT_LEFT_DOWN(CustomButton::OnMouseLeftDown)
                    EVT_LEFT_UP(CustomButton::OnMouseLeftUp)
                        EVT_BUTTON(wxID_ANY, CustomButton::OnButtonClick)
                            wxEND_EVENT_TABLE()
    ...
    SetFont(wxFont(14, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_MEDIUM, false, "SF Pro Display"));
...
void CustomButton::OnPaint(wxPaintEvent& event) {
    ...
    int borderRadius = 2;  // Puedes ajustar este valor para hacer las esquinas más o menos redondeadas
